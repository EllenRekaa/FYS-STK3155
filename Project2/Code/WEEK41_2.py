...
y = np.exp(-x**2) + rnd.normal(0, 0.1, x.shape) + 1.5 * np.exp(-(x-2)**2)
# simpler 2nd degree poly to fit
y = 0.5 * x ** 2 + rnd.normal(0, 0.1, x.shape)

maxd = 3 # maximum no. of degrees in the design matrix

const = 1e-4 # RIDGE CONST

X = np.column_stack([x**i for i in range(maxd)]) # design matrix
...
plt.plot(x, X @ trial_theta, linestyle = ':')
plt.plot(x, y, 'ro', label = 'Target', alpha = 0.2)
